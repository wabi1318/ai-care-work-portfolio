...
import { ArrowLeft, CheckSquare, Check, Info, Loader2 } from "lucide-react";
import { toast } from "@workspace/ui/hooks/use-toast";
import { useSearchParams } from "next/navigation";
import { getCookie, setCookie } from "cookies-next";

// 実際の設定コンポーネント
function TaskSettingsContent() {
  ...
  useEffect(() => {
    // 接続状態をCookieから取得
    const connectionStatus = getCookie("tasks_connected");
    ...
    if (success) {
      toast({
        title: "Google Taskと連携しました",
        description: "Google Taskとの連携が完了しました。",
      });
    }
  ...
  const handleSaveSettings = () => {
    // 設定をCookieに保存
    setCookie("tasks_sync_enabled", syncEnabled);
    setCookie("tasks_auto_suggest", autoSuggestEnabled);
    setCookie("tasks_privacy_read_only", privacySettings.readOnly);
    setCookie("tasks_privacy_limited_access", privacySettings.limitedAccess);
    setCookie("tasks_privacy_anonymize", privacySettings.anonymizeData);

    toast({
      title: "設定を保存しました",
      description: "Google Task連携の設定を保存しました。",
    });
  };
        ...
        <div className="max-w-2xl mx-auto">
          <Card className="mb-6">
            <CardHeader>
              ...
              <CardDescription>
                Google Taskと連携して、タスクからケア活動を自動的に抽出します。
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-medium">Google Task接続状態</h3>
                    <p className="text-sm text-gray-500">
                      {isConnected ? "Google Taskと連携中" : "未接続"}
                    </p>
                  </div>
                  {!isConnected ? (
                    <Button
                      ...
                      <div className="flex items-center space-x-2">
                        <Switch
                          ...
                          onCheckedChange={(checked) =>
                            setPrivacySettings((prev) => ({
                              ...prev,
                              limitedAccess: checked,
                            }))
                          }
                        />
                        <Label htmlFor="limited-access">
                          直近2週間のタスクのみアクセス
                        </Label>
                      </div>
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-blue-600 mt-1">
                          タスクデータは安全に処理され、ケア活動の抽出にのみ使用されます。
                          いつでも連携を解除できます。詳細は
          ...
          {isConnected && (
            <div className="text-center">
              <Button asChild className="bg-rose-600 hover:bg-rose-700">
                <Link href="/calendar/tasks/insights">
                  <CheckSquare className="mr-2 h-4 w-4" />
                  Google Taskからケア活動を抽出
                </Link>
              </Button>
            </div>
          )}
